...
def evaluator(model, testenc, dev, args):
    ...
    # testenc.input_ids = testenc.input_ids[:,:2048]
    # testenc.attention_mask = testenc.attention_mask[:,:2048]
    # Convert the whole text of evaluation dataset into batches of sequences.
    input_ids = testenc.input_ids  # (1, text_len)
    nsamples = input_ids.numel() // 2048 #model.seqlen  # The tail is truncated.
    input_ids = input_ids[:, :nsamples * 2048].view(nsamples, 2048).to(dev)  # (nsamples, seqlen)
    ...
    inps = torch.zeros(
        (nbatches, batch_size, 2048, model.config.hidden_size), dtype=dtype, device=dev
    )
    inps = [0] * nbatches
