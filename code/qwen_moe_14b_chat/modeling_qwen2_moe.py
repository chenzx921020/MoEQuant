...
class Qwen2MoeSparseMoeBlock(nn.Module):
    def __init__(self, config):
        ...
        # if True:
        self.top1_cnt = torch.zeros(4,60)
        self.top2_cnt = torch.zeros(4,60)
        self.top3_cnt = torch.zeros(4,60)
        self.top4_cnt = torch.zeros(4,60)
    ...
    def forward(self, hidden_states: torch.Tensor) -> torch.Tensor:
        ...
        if self.save_expert_cnt:
            for m in range(selected_experts.shape[0]):
                ...
                self.top3_cnt[idx,selected_experts[m][2]] = self.top3_cnt[idx,selected_experts[m][2]]+1
                self.top4_cnt[idx,selected_experts[m][3]] = self.top4_cnt[idx,selected_experts[m][3]]+1
