...
class MixtralBlockSparseTop2MLP(nn.Module):
    def __init__(self, config: MixtralConfig):
        ...
        self.w1.cur_sample = 0
        self.w2.cur_sample = 0
        self.w3.cur_sample = 0
...
class MixtralSparseMoeBlock(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.top1_cnt = torch.zeros(4,8)
        self.top2_cnt = torch.zeros(4,8)
    ...
    def forward(self, hidden_states: torch.Tensor, cur_sample: int=0) -> torch.Tensor:
        """ """
        hidden_states = hidden_states.to(self.experts[0].w1.weight.device)
        ...
        if self.save_expert_cnt:
            for m in range(selected_experts.shape[0]):
                idx = m//sequence_length
                self.top1_cnt[idx,selected_experts[m][0]] = self.top1_cnt[idx,selected_experts[m][0]]+1
                self.top2_cnt[idx,selected_experts[m][1]] = self.top2_cnt[idx,selected_experts[m][1]]+1
        ...
        for expert_idx in range(self.num_experts):
            ...
            expert_layer.w1.cur_sample = cur_sample
            expert_layer.w2.cur_sample = cur_sample
            expert_layer.w3.cur_sample = cur_sample
...
class MixtralDecoderLayer(nn.Module):
    ...
    def forward(
        ...
        **kwargs,
        ...
        hidden_states, router_logits = self.block_sparse_moe(hidden_states,cur_sample)
        hidden_states = residual + hidden_states
